use crate::{network::NamedCollective, sharding::{SeqModelSpec, ShardStrategy}, utils::ValidationError};
...
where O1: Operation,
    ...
    ) -> (Vec<super::ComputeUnit>, Option<crate::network::NamedCollective>) {
    ...
    fn memory_bytes(&self, axes: &crate::sharding::SeqModelSpec, strategy: &crate::sharding::ShardStrategy) -> super::MemoryProfile {
        self.op1.memory_bytes(axes, strategy).combine(&self.op2.memory_bytes(axes, strategy))
    }
    
    fn validate(&self, axes: &crate::sharding::SeqModelSpec, strategy: &crate::sharding::ShardStrategy) -> Result<(), ValidationError> {
        self.op1.validate(axes, strategy).and_then(|_| self.op2.validate(axes, strategy))
    }
}